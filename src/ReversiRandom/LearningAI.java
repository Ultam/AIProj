...
class LearningAI {
    ...
    Network network = FeedForwardNetwork.load(new File("network.ffn"));
    NetworkExecutor executor = new NetworkExecutor(network, new SimpleKernelExecutor());

    Integer[] indices = new Integer[64];
    ...
    public LearningAI(int _me, String host) throws IOException {
        me = _me;
        initClient(host);
...
//                frame.invalidate();
    ...
    private int move() {
        ...
        setInputs();
        graphics.drawImage(ReversiAI.inputsToImage(input), 0, 0, 256, 512, null);
        frame.invalidate();
//        try {
//            Thread.sleep(200);
...
//            e.printStackTrace();
//        }
        float[] outputs = executor.forward(input);

        for (int i = 0; i < indices.length; i++)
            indices[i] = i;

        Arrays.sort(indices, (x, y) -> Float.compare(outputs[y], outputs[x]));

        for (int i = 0; i < indices.length; i++) {
            for (int j = 0; j < numValidMoves; j++) {
                if (indices[i] == validMoves[j])
                    return j;
            }
        }
    ...
    private void setInputs() {
        ...
        for (int y = 0; y < 8; y++)
            for (int x = 0; x < 8; x++)
                input[index++] = state[x][y] == 1 ? 1 : 0;

        for (int y = 0; y < 8; y++)
            for (int x = 0; x < 8; x++)
                input[index++] = state[x][y] == 2 ? 1 : 0;
    }
    ...
    public static void main(String args[]) {
        try {
            new LearningAI(Integer.parseInt(args[1]), args[0]);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
